"""
Alphalens 因子有效性分析 V1 (过滤涨停版)

在原始版本基础上，增加了涨停股过滤功能

使用方法：
    python alphalensv1.py --exp-id test_001_fined_v1
"""
...
def run_alphalens_analysis(exp_id: str, periods: tuple = (20,), quantiles: int = 10):
    """
    运行 Alphalens 因子分析（过滤涨停版）
    ...
    """
    print("=" * 70)
    print(f"Alphalens 因子分析 V1 (过滤涨停版) - {exp_id}")
    print("=" * 70)
    
    # 1. 加载数据
    print("\n[1/4] 加载数据...")
    predictions = load_predictions(exp_id)
    ...
    limit_up_mask = ret >= 0.099  # 涨幅 >= 9.9% 认为是涨停
    
    # 向量化操作：使用 merge 代替循环
    # 将 limit_up_mask 转换为 long format
    limit_up_df = limit_up_mask.reset_index().melt(
        id_vars=['time'], 
        var_name='stock_code', 
        value_name='is_limit_up'
    )
    limit_up_df = limit_up_df[limit_up_df['is_limit_up'] == True]
    limit_up_df = limit_up_df.rename(columns={'time': 'date'})
    
    # merge 并标记
    predictions = predictions.merge(
        limit_up_df[['date', 'stock_code']], 
        on=['date', 'stock_code'], 
        how='left', 
        indicator='is_limit_up'
    )
    predictions['is_limit_up'] = predictions['is_limit_up'] == 'both'
    
    # 统计
    n_total = len(predictions)
    n_limit_up = predictions['is_limit_up'].sum()
    
    # 剔除涨停股
    predictions_filtered = predictions[~predictions['is_limit_up']].copy()
    predictions_filtered = predictions_filtered.drop('is_limit_up', axis=1)
    
    print(f"  原始记录: {n_total}")
    print(f"  涨停股: {n_limit_up} ({n_limit_up/n_total:.2%})")
    print(f"  过滤后: {len(predictions_filtered)}")
    
    predictions = predictions_filtered
    # ===== 过滤结束 =====
    
    # 2. 准备 Alphalens 格式数据
    print("\n[3/4] 准备 Alphalens 数据格式...")
    ...
    report_dir = ensure_report_dir(exp_id + '_v1_filtered')  # 不同的报告目录
    generate_alphalens_report(factor_data, ic, mean_return_by_q, turnover, 
                               report_dir, periods, quantiles, n_limit_up)
...
def generate_alphalens_report(factor_data, ic, mean_return_by_q, turnover,
                               report_dir: str, periods: tuple, quantiles: int, n_filtered: int = 0):
    ...
    ax1.set_title(f'IC Time Series ({period_str}) - Filtered', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f"{report_dir}/ic_analysis.png", dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  [OK] IC分析图: ic_analysis.png")
    ...
    ax1.set_title('Mean Return by Quantile (Filtered)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f"{report_dir}/returns_analysis.png", dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  [OK] 收益分析图: returns_analysis.png")
    ...
    plt.tight_layout()
    plt.savefig(f"{report_dir}/turnover_analysis.png", dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  [OK] 换手率分析图: turnover_analysis.png")
    
    # 4. 生成 HTML 报告
    generate_html_report(ic, mean_return_by_q, turnover, report_dir, periods, quantiles, n_filtered)
    print(f"  [OK] 综合报告: alphalens_report.html")


def generate_html_report(ic, mean_return_by_q, turnover, report_dir, periods, quantiles, n_filtered=0):
    """生成 HTML 摘要报告"""
...
<head>
    <meta charset="UTF-8">
    <title>Alphalens Report V1 (Filtered) - {datetime.now().strftime('%Y-%m-%d')}</title>
    <style>
        ...
        .filter-info {{ background: #fff3e0; padding: 15px; border-radius: 8px; margin: 10px 0; 
                        border-left: 4px solid #FF9800; }}
    </style>
</head>
<body>
    <div class="container">
        <h1> Alphalens 因子分析报告 V1 (过滤涨停版)</h1>
        <p>生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        
        <div class="filter-info">
            <h3> 涨停过滤信息</h3>
            <p>本报告已剔除涨幅 >= 9.9% 的涨停股票</p>
            <p>剔除涨停股数量: <strong>{n_filtered}</strong></p>
            <p>剩余有效样本用于分析</p>
        </div>
        ...
        <h2> Rank IC 分析</h2>
        <img src="ic_analysis.png" alt="IC Analysis">
        <div class="explanation">
            <strong>说明：</strong>Rank IC 是 Spearman 相关系数，衡量预测分数排名与未来收益排名的相关性。IC > 0.05 为优秀，IR > 0.5 为稳定。
        </div>
        
        <h2> 收益分析</h2>
        <img src="returns_analysis.png" alt="Returns Analysis">
        ...
        <h2> 换手率/稳定性分析</h2>
        <img src="turnover_analysis.png" alt="Turnover Analysis">
        <div class="explanation">
        ...
        <p style="color: #999; font-size: 12px; margin-top: 30px;">
            Generated by Alphalens V1 (Filtered) | Data: {datetime.now().strftime('%Y-%m-%d')}
        </p>
    </div>
</body>
</html>
    ...
    with open(f"{report_dir}/alphalens_report.html", 'w', encoding='utf-8') as f:
        f.write(html)


def main():
    parser = argparse.ArgumentParser(description='Alphalens 因子有效性分析 V1 (过滤涨停版)')
