...
**方案**：三张报表独立处理，各自按实际公告日（f_ann_date）推进版本状态

- 不同公告日的版本全部保留，后续修订只从自己的公告日之后替换旧版本；
- 同日 type 1/type 5 冲突时保守选择 type 5 调整前原始值；
- 晚于首个 type 1 的 type 5 仅视为旧值归档，不生成倒退事件；
...
**关键原则**：公告当天仍使用旧值，版本从公告后的第一个交易日生效。
